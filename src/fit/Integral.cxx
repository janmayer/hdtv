...
// Access functions (cached)
double Integral::GetIntegral()          { static double val = CalcIntegral(); return val; }
double Integral::GetIntegralError()     { static double val = CalcIntegralError(); return val; }
double Integral::GetMean()              { static double val = CalcMean(); return val; }
double Integral::GetMeanError()         { static double val = CalcMeanError(); return val; }
double Integral::GetVariance()          { static double val = CalcVariance(); return val; }
double Integral::GetVarianceError()     { static double val = CalcVarianceError(); return val; }
double Integral::GetRawSkewness()       { static double val = CalcRawSkewness(); return val; }
double Integral::GetRawSkewnessError()  { static double val = CalcRawSkewnessError(); return val; }
double Integral::GetSkewness()          { static double val = CalcSkewness(); return val; }
double Integral::GetSkewnessError()     { static double val = CalcSkewnessError(); return val; }
...
double Integral::CalcMeanError()
{
  ...
  //! \f[ \Delta \bar{x} = \frac{1}{N}   \sqrt{\sum_{i=b_{1}}^{b_{2}} (x_{i} - \bar{x})^{2} (\Delta n_{i})^{2}} \f]
  //! calculated from the bin errors by Gaussian error propagation
...
double Integral::CalcVariance()
{
  ...
  //! \f[ \sigma^{2} = \frac{1}{N} \sum_{i=b_{1}}^{b_{2}} (x_{i} - \bar{x})^{2} n_{i} \f]
...
double Integral::CalcVarianceError()
{
  ...
  //! \f[  \Delta \sigma^{2} = \frac{1}{N}   \sqrt{\sum_{i=b_{1}}^{b_{2}} [(x_{i} - \bar{x})^{2} - \sigma^{2}]^{2} (\Delta n_{i})^{2} } \f]
  //! calculated from the bin errors by Gaussian error propagation
...
double Integral::CalcRawSkewness()
{
  ...
  //! moment)
  //! \f[ \mu_{3} = \frac{1}{N} \sum_{i=b_{1}}^{b_{2}} (x_{i} - \bar{x})^{3} n_{i} \f]
...
double Integral::CalcRawSkewnessError()
{
  ...
  //! \f[ \Delta \mu_{3} = \frac{1}{N}   \sqrt{\sum_{i=b_{1}}^{b_{2}} [(x_{i} - \bar{x})^{3} - 3 \sigma^{2} (x_{i} - \bar{x}) - \mu_{3}]^{2} (\Delta n_{i})^{2}} \f]
  //! calculated from the bin errors by Gaussian error propagation
...
double Integral::CalcSkewnessError()
{
  ...
  //! \f[ \Delta \gamma = \frac{1}{N}   \sqrt{ \sum_{i=b_{1}}^{b_{2}} \left[ \frac{(x_{i} - \bar{x})^{3}}{\sigma^{3}} - 3 \frac{x_{i} - \bar{x}}{\sigma} - \frac{3}{2} \gamma \frac{(x_{i} - \bar{x})^{2}}{\sigma^{2}} - \frac{1}{2} \gamma \right]^{2} (\Delta n_{i})^{2} } \f]
  //! calculated from the bin errors by Gaussian error propagation
...
TH1Integral::TH1Integral(TH1* hist, double r1, double r2)
  : Integral(hist->FindBin(r1), hist->FindBin(r2)),
    fHist(hist)
{
...
BgIntegral::BgIntegral(const Background* background, double r1, double r2, TAxis* axis)
  : Integral(axis->FindBin(r1), axis->FindBin(r2)),
