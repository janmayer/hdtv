...
def find_peaks(hist):
	...
	npk = spec.Search(hist, 2.0, "goff", 0.02)
...
def fit_peak(hist, pos, left_neighbour, right_neighbour):
	...
	fit_pos = par[1]
	fit_vol = math.sqrt(2 * math.pi) * par[0] * abs(par[2])
...
for det in [4]:
	hist = SpecReader().Get("/mnt/braun/88Zr_angle_singles/0075/ge%d.0075" % det, "spec_%d" % det, "spec")
	sigfile = open("data/ge%d.sig" % det, 'w')
	...
	for i in range(0, len(fit_peaks)):
		fit_pos = fit_peaks[i][0]
		fit_vol = fit_peaks[i][1]
		if(fit_pos > 500 and fit_vol > 10000):
			print "%8.2f %8.0f" % (fit_pos, fit_vol)
			sigfile.write(str(fit_pos) + " ")
