...
class EffCalHDTVInterface(object):
    ...
    def __init__(self, EffCalIf):
        ...
        hdtv.cmdline.AddCommand(
            prog, self.ReadPar, parser=parser, fileArgs=True, nargs=1)
        ...
        parser.add_option(
            ...
            default="active")
        hdtv.cmdline.AddCommand(
            prog, self.ReadCov, parser=parser, fileArgs=True, nargs=1)
        ...
        hdtv.cmdline.AddCommand(prog, self.WritePar,
                                parser=parser, fileArgs=True, nargs=1)
        ...
        parser = hdtv.cmdline.HDTVOptionParser(
            prog=prog, description=description, usage=usage)
        hdtv.cmdline.AddCommand(
            prog, self.PlotEff, parser=parser, fileArgs=False, nargs=1)
        ...
        usage = "%prog <spectrumID0>[,<nuclide0>,<factor> <spectrumID1>,<nuclide1>,<factor> ...]\n-g (--show-graph)\n-p (--fit-panel)\n-t (--show-table)\n--sigma=sigma\n--database=database\nExample: calibration efficiency fit 4,Ra-226,1e7 3,Eu-152 1,Ba-133 -gpt"
        parser = hdtv.cmdline.HDTVOptionParser(
            prog=prog, description=description, usage=usage)
        parser.add_option(
            "-f",
            ...
            action="store",
            default=None)  # does not really make sens, see method "Fit"
        parser.add_option("-p", "--fit-panel", help="Show fit panel",
                          action="store_true", default=False)
        parser.add_option("-g", "--show-graph", help="Show fitted graph",
                          action="store_true", default=False)
        parser.add_option(
            "-t",
            "--show-table",
...
class EnergyCalIf(object):
    ...
    def CalFromPairs(self, pairs, degree=1, table=False,
                     fit=False, residual=False, ignoreErrors=False):
        """
...
class EnergyCalHDTVInterface(object):
    ...
    def __init__(self, ECalIf):
        ...
        parser = hdtv.cmdline.HDTVOptionParser(
            prog=prog, description=description, usage=usage)
        hdtv.cmdline.AddCommand(prog, self.CalPosCopy,
        ...
        hdtv.cmdline.AddCommand(
            ...
            parser=parser,
            minargs=0,
            fileargs=True)  # Number of args can be 0 if "-f" is given
        ...
        parser = hdtv.cmdline.HDTVOptionParser(prog=prog, usage=usage)
        hdtv.cmdline.AddCommand(
            prog, self.CalPosListClear, parser=parser, nargs=0)
    ...
    def CalPosEnter(self, args, options):
        ...
        try:
            ...
            if options.file is not None:  # Read from file
    ...
    def CalPosNuc(self, args, options):
        ...
        if args == []:
            raise hdtv.cmdline.HDTVCommandError(
                "You must name at least one nuclide.")
        ...
        for nucl in nuclide:
            for Energy in EnergyCalibration.SearchNuclide(nucl, options.database)[
                    0]:
                Energies.append(Energy)

        database = str(EnergyCalibration.SearchNuclide(
            nucl, options.database)[6])

        # matches the right peaks with the right energy
        Match = EnergyCalibration.MatchPeaksAndEnergies(Peaks, Energies, sigma)
    ...
    def CalPosRead(self, args, options):
        ...
        try:
            ...
            hdtv.ui.error(
                "Malformed calibration parameter file \'%s\'." % fname)
            return False
        except IOError as msg:
            hdtv.ui.error(str(msg))
            return False
            ...
            if len(args) % 2 != 0:
                ...
                for i in range(0, len(args), 2):
                    ID = hdtv.util.ID.ParseIds(
                        args[i], spec, only_existent=False)[0]
